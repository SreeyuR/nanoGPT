...
class WikiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        """
        Arguments: an index `idx`
        Returns: a data point (x, y) where x and y are Long tensors of length self.block_size.
                 x encodes the input sequence, and y encodes the output sequence.
        """
        ...
        if not self.char_corruption:
            ...
            if len(document) != 0:
                masked_expected_len = len(document) // 4
                # Sample from uniform distribution with expectation 1/4
                masked_len = random.randint(1, 2 * masked_expected_len - 1)
                prefix_len = random.randint(1, len(document) - masked_len)
            ...
            # 3. Rearrange these substrings into the form: [prefix] MASK_CHAR [suffix] MASK_CHAR [masked_content] [pads]
            masked_str = f"{prefix}{self.MASK_CHAR}{suffix}{self.MASK_CHAR}{masked_content}"
            masked_str += f"{self.PAD_CHAR * (self.block_size - len(masked_str))}"

            # 4 and 5. Use masked_string to construct the input and output example pair.
            input_string = torch.tensor([self.stoi[s] for s in masked_str[:-1]], dtype=torch.long)
            output_string = torch.tensor([self.stoi[s] for s in masked_str[1:]], dtype=torch.long)
